...
export class VaultManager {
    defaultVaultPath;
    frontmatterFailureCount = 0;
    frontmatterFailureLimit = 10;
    constructor() {
    ...
    async auditRecentNotes(options = {}) {
        const { hoursBack = 72, limit = 25, requiredFields = ['title', 'created'], requireHeadings = false, vaultPath, } = options;
        const vault = this.getVaultPath(vaultPath);
    ...
    async generateContextualCompanions(options) {
        ...
        for (const candidate of candidatePaths) {
            ...
            companions.push({
                ...
                linkTypes: Array.from(reasons.get(candidate) || []).map((text) => text.startsWith('Linked') || text.startsWith('References') ? 'link' : 'keyword'),
            });
        }
        companions.sort((a, b) => b.score - a.score);
        return companions.slice(0, Math.max(1, limit));
    }
    async findFreshEnergyNotes(options = {}) {
    ...
    async bridgeInitiativeOpportunity(options) {
    ...
    async findSynthesisReadyClusters(options = {}) {
    ...
    async generateStoryPath(notePath, options = {}) {
        const { supportingLimit = 3, counterpointLimit = 3, includeActionItems = true, vaultPath, } = options;
        const vault = this.getVaultPath(vaultPath);
        const fullPath = path.join(vault, notePath);
        let rawSeedContent;
        try {
    ...
    async resolveNoteReference(reference, vault) {
        ...
        const exactMatch = fallbackMatches.find((candidate) => this.noteKey(candidate) === normalizedTarget);
        return exactMatch || fallbackMatches[0];
    }
    noteKey(input) {
        return input.replace(/\\/g, '/').replace(/\.md$/i, '').toLowerCase();
    }
    extractLinkContext(content, reference) {
        const lines = content.split('\n');
    ...
    extractActionItems(body, limit = 5) {
    ...
    formatContextForBlockquote(context) {
    ...
    normalizeContextForExcerpt(context) {
        return context
            .split('\n')
    ...
    hasFrontmatterField(data, field) {
    ...
    extractTagsFromFrontmatter(data) {
    ...
    calculateKeywordOverlap(keywords, content) {
    ...
    calculateTokenOverlap(snippet, content) {
        if (!snippet || !content) {
            return 0;
        }
    ...
    bumpScore(map, key, amount) {
        map.set(key, (map.get(key) || 0) + amount);
    }
    appendReason(map, key, reason) {
        if (!map.has(key)) {
            map.set(key, new Set());
        }
    ...
    findMatchingLines(lines, snippet, keywords, maxLines = 3) {
    ...
    valueMatchesInitiative(value, initiative) {
        if (!value) {
            return false;
        }
        if (typeof value === 'string') {
        ...
        if (Array.isArray(value)) {
            return value.some((item) => typeof item === 'string' && item.trim().toLowerCase() === initiative);
        }
        return false;
    }
    async summaryNoteExists(cluster, vault) {
        for (const notePath of cluster) {
            ...
            const looksLikeSummary = titleLower.includes('summary') ||
